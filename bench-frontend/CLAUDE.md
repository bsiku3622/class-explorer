...
**여기는 `bench-frontend/` 입니다.** 전교생에게 열 ksa-bench 이고, 백엔드는
class-explorer 와 **같은 서버**(`backend.main:app`)를 봅니다.

🧊 **동결 상태입니다. 여기에 새 기능을 넣지 마세요.**

개발은 `frontend/`(class-explorer)에서만 합니다. 전교생에게 열 때가 오면 그때
class-explorer 에서 **문제가 되는 지점만 골라** 여기로 포장합니다.

이 디렉토리는 그 작업의 **참고 자료**입니다 — 훑는 UI 를 걷어내고 명단 캐시를 없앤
상태가 어떤 모양인지 이미 한 번 만들어 본 결과가 남아 있습니다.
...
| | class-explorer | 여기 |
| --- | --- | --- |
| 분반 → 학생 명단 | 있음 | 있음 — Trade 가 필요로 합니다 |
| localStorage 캐시 | 학기 데이터 1시간 | **안 합니다** — 메모리에만 |
...
**1. 훑는 화면을 없앤다.** 학교 공식 앱(가온누리)에도 전교생 시간표 검색이 있고 학번이
연속이라 순회하면 긁히므로, 데이터를 완전히 막는 건 의미가 없습니다. 대신 한 번에 여러
명이 나오는 경로를 없앱니다 — 전교생 목록 화면도, 다중 검색도 없습니다.

**2. 명단을 브라우저에 안 남긴다.** `GET /` 응답에는 분반 명단이 들어 있습니다(Trade 가
필요로 합니다). class-explorer 는 이걸 1시간 캐시하지만 **여기서는 캐시하지 않습니다** —
localStorage 에 넣는 순간 전교생 명단이 파일로 남습니다. 대가는 새로고침할 때마다 다시
받는 것뿐입니다.
...
- 학번 분포(`year_counts`)는 **의도적으로 남긴 것**입니다 — 분포만으로는 누구인지
  좁혀지지 않습니다
- **수업 데이터를 localStorage 에 넣지 마세요.** 이 앱이 class-explorer 와 갈리는
  두 지점 중 하나입니다. 편의를 위해 캐시를 되살리고 싶어지면, 응답에 명단이 들어
  있다는 것부터 다시 보세요
...
**이 앱은 배포하지 않습니다** (동결). 나중에 올릴 때 환경변수를 잡으면 됩니다.
...
수업 데이터를 localStorage 에 넣지 않습니다. 응답에 분반 명단이 들어 있어서, 캐시하면
전교생 명단이 브라우저에 파일로 남습니다. 데이터는 React state 에만 있고 탭을 닫으면
사라집니다. 그래서 `CACHE_VERSION`·`CACHE_EXPIRY` 도 없습니다.

`clearDataCache()` 는 **class-explorer 를 쓰던 브라우저에 남은 옛 캐시를 지우려고만**
남겨 뒀습니다 — 시작할 때와 로그아웃할 때 부릅니다.

localStorage 에 남는 것: 세션 토큰, 선택 학기(`ksa_selected_term`), 검색 히스토리.
