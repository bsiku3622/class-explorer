...
**ksa-bench 에서 없어진 것**: 분반 → 학생 **이름** 목록, 다중 검색·불린 연산·초성,
`/browse` 학생 목록, `/trade`, `/admin`. 학생 검색은 **한 번에 한 명**만 되고
(후보 목록 → 하나 선택), 교사·강의실 검색은 이름 없이 남습니다.
인원수와 **학번 분포는 그대로 줍니다** — 선은 "이름이 안 나간다" 입니다.
