...
### `POST /auth/google`
학교 구글 계정으로 로그인합니다. 프론트가 Google Identity Services에서 받은 ID 토큰을
그대로 넘깁니다.

```json
{ "credential": "<google id token>", "device_type": "web" }
```

이메일이 곧 학번이라(`25-059@ksa.hs.kr`) 따로 대조하지 않습니다. 처음 들어오면 계정이
자동으로 만들어지고, 같은 학번을 쓰는 옛 계정이 있으면 그 계정에 이어붙여 기록을
남깁니다.

| 응답 | 경우 |
|------|------|
| `200` | 로그인 성공 — 세션 토큰 |
| `401` | 토큰이 우리 앱 것이 아니거나 만료됨 |
...
아이디·비밀번호로 들어온 옛 계정에 학교 구글 계정을 붙입니다. 프론트는 `email`이
비어 있는 계정에 이 창을 강제로 띄우고, 연결 전에는 앱을 쓸 수 없게 막습니다.
...
| `403` | `@ksa.hs.kr` 학번 계정이 아님 / 명단에 없는 학번 |
| `409` | 다른 계정이 쓰는 구글 계정 / 이 계정에 이미 다른 학번이 등록됨 |

이미 학번이 정해진 계정이면 구글 계정의 학번과 같아야 합니다 — 다르면 남의 계정에
붙이려는 것이므로 막습니다.

---

### `POST /auth/link-student` *(인증 필요)*
계정에 본인 학번을 등록합니다. **학번과 이름이 함께 맞아야** 합니다.

```json
{ "stu_id": "25-059", "name": "백재원" }
```

| 응답 | 경우 |
|------|------|
| `200` | 등록 성공 — `{ "stu_id": "...", "student_name": "..." }` |
| `400` | 학번이 없거나 이름이 다름 (어느 쪽이 틀렸는지는 알려주지 않음) |
| `409` | 이미 다른 계정이 쓰고 있는 학번 (동시 요청도 DB 유니크 제약으로 막힘) |

아무 학번이나 골라 남의 이름으로 성적을 기록해 두는 걸 막으려는 검증이라, 둘 중 하나만
맞아도 반려합니다. 이름은 띄어쓰기 차이만 무시합니다.
