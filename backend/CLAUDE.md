...
⚠️ **수업이 없는 날에는 `today` 를 비웁니다.** 학기 데이터는 `ClassTime.day` 로 요일만
들고 있어서 **날짜를 모릅니다** — 그냥 조회하면 방학 중 월요일에도 월요일 시간표가
그대로 나옵니다. 그래서 학사일정으로 한 번 거르고(`off_reason`: `vacation`·`weekend`·
`holiday`), 걸리면 수업 질의를 아예 하지 않습니다. 휴업일은 `category='holiday'` 중
**공용 일정만** 봅니다(`owner_id IS NULL`) — 개인 일정이 남의 수업을 없애면 안 됩니다.
