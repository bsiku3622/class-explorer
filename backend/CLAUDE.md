...
`BREAKS`(점심·AA·저녁·자습·간식)는 **수업을 밀지 않습니다.** 점심·AA 는 교시 사이의 빈
자리지만 **저녁(17:30~19:00)·자습(19:30~21:30)은 수업과 동시에 진행**됩니다. 그래서
`current_period()` 와 `current_break()` 가 둘 다 값을 낼 수 있고, 화면은 "9교시 · 저녁"
처럼 같이 보여 줍니다.
