...
| `TodayTimeline` | `DayRuler` 가 그린 하루의 **글로 쓴 판본**. `divide-y divide-black/10` 로만 나누고 줄에 테두리를 두르지 않습니다. **줄 높이 = 교시 수 × 3.25rem** — 100분 연강은 50분 수업의 두 배고, 공강도 같은 한 칸입니다(같은 시간인데 높이가 다르면 시간을 재는 물건이 아니게 됩니다). 왼쪽에 **학과색 띠**, 과목명도 학과색이며 진행 중이면 면·띠·글자가 모두 핑크로 모입니다. **연강은 자·주간 격자와 같은 조건으로 잇고**(과목·분반·교실·시간 인접) 교시는 `10–11교시` 로 적습니다. **빈 시간을 건너뛰지 않습니다** — 수업만 늘어놓으면 2교시 다음이 5교시인 게 안 보입니다. ⚠️ **공강에 "지금" 을 붙이지 않습니다** · **글자 하한 12px** |
