...
| `home/WeekTimetable` | 홈 아래에 붙는 **주간 격자** — 위 카드가 오늘을 말한다면 여기는 **이 학기**입니다. 그래서 `today` 와 달리 **방학·주말에도 그립니다**(서버가 `week` 를 안 비웁니다). **표가 아니라 지도입니다** — 수업은 `border-2 border-black bg-white` **칩**이고 빈 자리는 진짜 비웁니다(검은 덩어리 = 수업). 이름도 교시 번호도 **위쪽 정렬**이라 연강의 이름이 시작 교시와 일직선입니다. 교시 사이가 20분 넘게 벌어지면(점심 70분·저녁 60분) **빗금 띠**를 끼우고 **그 위로는 연강을 잇지 않습니다**. 잇는 조건은 과목·분반·**교실**·**시간 인접** 넷 다입니다. 이름은 꼬리 괄호를 떼고(`융합특강(AI시대의인간기술)` → `융합특강`, `(EC)` 도 뗍니다 — 내 시간표라 가릴 게 없습니다) 두 줄까지 폅니다. 교실은 **이름 바로 아래**(`mt-auto` 로 바닥에 밀면 연강에서 아래 교시 것처럼 읽힙니다)이고 **좁은 화면에서도 남깁니다**. **면을 채우는 건 "지금" 칩 하나뿐** — 오늘 표시는 색 없이 검은 요일 바에서 그 칸만 **흰색으로 반전**합니다(핑크로 칠하면 머리와 칩이 같은 색이 됩니다). 행 높이는 **좁은 쪽이 더 큽니다**(52px vs 44px) — 폭이 없으면 높이로 갚습니다 |
