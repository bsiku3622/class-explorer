...
| `GoogleLoginButton` | 학교 구글 계정 로그인 버튼 (`onCredential`, `onError`). 생김새는 인풋과 같게 우리가 그리고, 구글이 그린 버튼을 투명하게 그 위에 겹쳐 둡니다 — 눌리는 건 구글 것이라 ID 토큰 흐름을 그대로 씁니다. `VITE_GOOGLE_CLIENT_ID`가 없으면 아무것도 그리지 않습니다 |
