...
| `home/TodayCardV3` | 홈 배치 **V3 — 배포본**. **머리 한 줄**(지금 \| 오늘 일정 \| 급식) + **본문 두 칸**(주간 격자 \| 오늘). `WeekTimetable` 을 **자기가 그립니다** — 오늘과 나란히 놓아야 해서 `HomePage` 아래에 따로 붙이면 어긋납니다. DOM 은 오늘이 먼저고 `xl:order-2` 로 넓을 때만 자리를 바꿉니다(좁으면 한 주가 오늘 위로 올라갑니다). 급식은 **머리 줄**입니다 — 아래로 내리면 자·목록 사이에 끼어 흐름이 끊기고, `meal` 이 `null` 이어도 칸을 남겨 "급식 정보가 없습니다" 를 띄웁니다. 히어로는 목록이 못 하는 말만 합니다 — 남은 시간(진행바)·다음 수업·**교실 이동**(`current.room !== next.room`), 그리고 **연강 단위**(`currentPeriods`)로 잡아야 20:20 에 끝난다면서 NEXT 에 같은 과목을 또 거는 일이 없습니다. `planned` 를 켜면 날짜 옆에 `계획 미리보기` 표식이 붙고 `WeekTimetable` 에도 그대로 넘깁니다 — 계획은 아직 등록된 시간표가 아니라서 어느 쪽을 보는지 헷갈리면 엉뚱한 교실로 갑니다 |
...
| `home/WeekTimetable` | 홈 본문 왼쪽의 **주간 격자** — 위 카드가 오늘이라면 여기는 **이 학기**입니다. 그래서 `today` 와 달리 **방학·주말에도 그립니다**. 칩은 **학과색**(테두리 100% + 채움 8%)이고 과목명·분반·교사·교실 세 층입니다. 행 높이는 **칸이 4:3** 이 되는 값(`h-24 md:h-28 xl:h-20`)이고, 점심·저녁 구멍은 **교시 칸에 비례**합니다(70분 점심이 16px 이면 50분 수업보다 짧아 보입니다). 요일 머리는 옅은 회색 바에 **오늘만 핑크**, 그 아래로 **핑크 기둥**이 열 전체를 관통합니다 — ⚠️ `gridRow: "2 / -1"` 은 행이 암시적이라 헤더 한 칸으로 쪼그라듭니다(`2 / span {행 수}`). 연강은 과목·분반·**교실**·**시간 인접** 넷이 다 맞아야 잇습니다. 격자만 제 안에서 스크롤하고(`max-h`), 머리의 **PNG 버튼**으로 내보냅니다(스크롤 상자가 아니라 **안쪽 격자**를 찍어야 잘린 교시까지 담깁니다). `planned` 면 제목 옆에 `계획` 표식이 붙고 PNG 이름에도 `_계획` 이 들어갑니다 — 찍어 둔 그림에는 홈 위쪽 표식이 안 담깁니다. 제목은 **`Timetable`** 이고, 칩의 **과목명·분반·교사는 검색 링크**입니다(`searchEngine` 의 `subjectQuery`·`sectionQuery`·`teacherQuery`). 머리의 `강조 없이` 체크박스를 켜면 **오늘 열·요일 머리·지금 칩의 핑크를 뺀 채로** PNG 를 찍습니다 — ⚠️ 상태를 켠 즉시 찍으면 강조가 담기므로 `requestAnimationFrame` 을 두 번 넘긴 뒤 찍습니다 |
...
| `home/MySubjects` | 홈 **맨 아래**의 수강 과목 목록. 격자가 "언제" 라면 여기는 **"무엇을"** 입니다 — `week` 를 과목 단위로 접어(`homeView.collectSubjects`) 과목 수·학점 합계를 머리에 답니다. **접힌 줄에도 분반·교사가 보입니다**(펼침은 교실·시간을 위한 자리 — 안 그러면 내 분반을 확인하려고 열두 줄을 다 엽니다). 학점을 모르는 과목이 있으면 합계에 `+` 를 붙입니다. 계획을 보는 중이면 **계획의 과목**이 나옵니다(`week` 를 그대로 받으므로 따로 알 필요가 없습니다) |
...
| `HomePage` | `/` | 홈 — **껍데기**(fetch·시계·DEV 스위치)이고 화면은 `home/TodayCardV3`(배포본)가 그립니다. V1·V2 는 개발에서만 되돌아볼 수 있습니다. 수강 정정 기간에 `/trade` 계획이 있으면 **[기존 시간표 \| 트레이드 계획]** 전환이 붙고, 고르면 **화면 전체**가 갈립니다(응답을 갈아 끼웁니다 — `lib/plannedHome.ts`). 고른 쪽은 `ksa_home_view` 로 이 기기에 남아 **다음에도 그대로 열립니다** |
