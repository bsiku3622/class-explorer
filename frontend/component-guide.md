...
| `SectionCard` | 개별 분반 카드 (학생 배지, 교사, 강의실, 시간). **명단은 전원 제 색으로 보입니다** — 검색어와 맞는 배지만 `animate-badge-bounce` 로 통통 튀고, ⚠️ 예전처럼 나머지를 `grayscale opacity-30` 으로 지우지 마세요(명단을 보는 이유의 절반이 "누구랑 같이 듣나" 입니다). 정지 화면에서도 구분되게 맞는 배지는 면을 더 채우고 그림자를 세게 둡니다 |
