...
로그인 화면은 탭 두 개입니다 — **Google**(학교 계정)과 **ID & PW**(옛 계정).

탭과 카드는 한 덩어리로 보여야 해서, 그림자는 둘을 감싼 바깥 div가 갖고
(`RetroCard shadow="none"`) 고른 탭은 아래 테두리를, 카드는 윗 테두리를 지웁니다.
흰 선으로 덮는 방식은 모서리에서 두 테두리가 대각선으로 물려 자국이 남습니다.
