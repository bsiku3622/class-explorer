...
`components/home/TodayCardV{1,2}.tsx` + `WeekTimetable.tsx` 가 그립니다. V1 은 되돌아볼 자리로 남겼고
...
### 주간 시간표 (`WeekTimetable`)

위 카드가 **오늘**을 말하는 자리라면, 그 아래 격자는 **이 학기**를 말합니다. 배치
판본(V1·V2)과 무관하게 같은 자리에 붙습니다.
...
- **연강은 한 덩어리입니다.** 같은 과목·분반·**교실**이 이어지면 행을 걸쳐 한 칸으로
  놓습니다. 교실까지 보는 이유는 사이에 이동이 있으면 묶으면 안 되기 때문입니다
...
- ⚠️ **수업 칸의 윗선만 진합니다**(`black/25`, 빈 칸은 `/10`). 같은 두께로 뒀더니 세로로
  붙은 서로 다른 수업이 옅은 회색 **한 덩어리**로 읽혀서, 연강을 이어 놓은 게 아무
  뜻이 없었습니다 — 이어 붙인 칸 안에는 선이 아예 없으므로 둘이 구별됩니다
- 행 높이는 `auto` 라 긴 과목명이 두 줄이 되면 그 줄만 늘어납니다. 자르는 것보다 낫습니다
