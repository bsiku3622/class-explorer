...
- ⚠️ **요청이 실패해도 화면을 지우지 않습니다.** 한동안 실패하면 상태를 `null` 로
  비웠는데, 1분마다 받는 화면이라 **노트북을 덮었다 열 때마다** 오류 카드가 떴습니다 —
  깨어난 직후 네트워크가 붙기 전에 주기 요청이 한 번 나가서 실패하기 때문입니다.
  실패는 위에 한 줄로 알리고(`연결이 끊겨 N분 전 받은 화면을 보여 주고 있습니다`)
  마지막 응답을 그대로 그립니다. **오류 카드는 한 번도 못 받았을 때만** 나옵니다
- **깨어나면 즉시 다시 받습니다** (`visibilitychange`·`online`). 덮어 둔 동안 타이머가
  멈춰 있어서, 안 그러면 다음 주기까지 최대 1분 동안 잠들기 전 시간표가 떠 있습니다
...
| **쉬는시간** | 교시와 교시 **사이**(20분 이하) | `쉬는시간` (`inBreak`) |
