...
```
[Trade 배너]          ← 수강 변경 기간에만. 핑크
[Now      |  Meal]    ← md 이상에서 2열, 높이는 서로 맞춥니다
[Today]
```

두 카드는 `h-full` + 안쪽 `flex-1` 로 높이를 맞춥니다 — 나란히 놓고 한쪽만 짧으면
어긋나 보입니다. Now 카드의 아래 줄(다음 수업·개학까지)은 그래서 바닥에 붙습니다.

**시계를 따로 두지 않습니다.** 큰 시각 하나가 맨 위를 차지하면 정작 "어디로 가야 하는가"
가 밀립니다 — 요일과 교시는 Now 카드 오른쪽 위에 작게 붙였습니다.
...
| 색 | 뜻 |
|---|---|
| `retro-accent1` (시안) | **지금** — 현재 수업 카드, 지금 시간대의 끼니, 지금 공강인 친구 |
| `retro-accent3` (핑크) | 지금만 열려 있는 **한시 기능** — Trade 배너 |
