...
**급식은 날짜를 앞뒤로 넘겨 봅니다** (`MealCard`). 메뉴는 **카드가 직접** `GET /meal` 로
받습니다 — 학교 API 가 3~5초씩 걸려서 홈 응답에 묶으면 그동안 홈 전체가 비어 있습니다.
따로 받으면 급식 칸만 기다립니다. **홈이 1분마다 다시 받아도 보고 있던 날짜는 그대로
둡니다** — 어제 급식을 읽는 중에 화면이 오늘로 튀면 안 됩니다.
