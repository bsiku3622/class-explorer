...
- ⚠️ **점심·저녁 구멍을 격자에 새깁니다.** 교시 시각표에서 20분 넘게 벌어진 자리를 찾아
  ...
  `학급모임`(20분)이 둘 다 걸쳐서, 처음 찾은 걸 쓰면 순서에 따라 답이 달라집니다
...
- **`강조 없이` 체크박스**를 켜면 오늘 열·요일 머리·지금 칩의 핑크를 **뺀 채로** 찍습니다.
  남에게 보내거나 인쇄하는 그림에서 "오늘"·"지금" 은 찍는 순간의 사정일 뿐이라, 다음 날
  부터는 틀린 표시가 됩니다. 세 자리가 같은 값(`markDay`)을 보므로 한 군데만 남지
  않습니다. ⚠️ **상태를 켠 즉시 찍으면 강조가 그대로 담깁니다** — React 가 다시 그리고
  브라우저가 칠할 때까지 `requestAnimationFrame` 을 두 번 넘긴 뒤에 찍습니다
...
홈의 마지막 카드는 **이 학기에 듣는 과목 목록**입니다. 위의 격자가 "언제" 를 묻는
물건이라 같은 과목이 요일마다 흩어져 있는데, 몇 과목인지·몇 학점인지 세는 건 목록이
훨씬 잘합니다 (`homeView.collectSubjects` 가 `week` 를 과목 단위로 접습니다).

- **접힌 줄에도 분반·교사가 보입니다.** 펼쳐야 알 수 있게 두면 "내가 몇 분반이지" 를
  확인하려고 열두 줄을 다 열게 됩니다 — 펼침은 **교실과 시간**을 위한 자리입니다
- **학점을 모르는 과목이 있으면 합계에 `+`** 를 붙입니다 (교육과정에 없는 과목은
  `credits` 가 `null` 입니다). 0으로 세고 조용히 더하면 틀린 합계가 확신에 차 보입니다
- 계획을 보는 중이면 **계획의 과목**이 나옵니다 — `week` 를 그대로 받으므로 이 카드는
  두 시간표를 따로 알 필요가 없습니다
